...
void loadTexture(char *filename, GLuint *texture) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texturePicData->ny, texturePicData->nx, 0, GL_RGB, GL_UNSIGNED_BYTE,
                 texturePicData->pix);
}
...
point calculateTagent(point P0, point P1, point P2, point P3) {
    ...
    double t = 0.01;
    double x = 0.5 * (-P0.x + P2.x + (2 * P0.x - 5 * P1.x + 4 * P2.x - P3.x) * (2 * t) +
                      (-P0.x + 3 * P1.x - 3 * P2.x + P3.x) * (3 * t * t));
    double y = 0.5 * (-P0.y + P2.y + (2 * P0.y - 5 * P1.y + 4 * P2.y - P3.y) * (2 * t) +
                      (-P0.y + 3 * P1.y - 3 * P2.y + P3.y) * (3 * t * t));
    double z = 0.5 * (-P0.z + P2.z + (2 * P0.z - 5 * P1.z + 4 * P2.z - P3.z) * (2 * t) +
                      (-P0.z + 3 * P1.z - 3 * P2.z + P3.x) * (3 * t * t));
    ...
    for (int i = 0; i < g_iNumOfSplines; i++) { //create proper # of splines
        ...
        for (int j = 0; j < g_Splines[i].numControlPoints - 3; j++) {
            ...
            for (double t = 0; t <= 1.0; t += 0.01) {
                point p;
                p.x = 0.5 * ((2 * P1.x) + (-P0.x + P2.x) * t + (2 * P0.x - 5 * P1.x + 4 * P2.x - P3.x) * pow(t, 2) +
                             (-P0.x + 3 * P1.x - 3 * P2.x + P3.x) * pow(t, 3));
                p.y = 0.5 * ((2 * P1.y) + (-P0.y + P2.y) * t + (2 * P0.y - 5 * P1.y + 4 * P2.y - P3.y) * pow(t, 2) +
                             (-P0.y + 3 * P1.y - 3 * P2.y + P3.y) * pow(t, 3));
                p.z = 0.5 * ((2 * P1.z) + (-P0.z + P2.z) * t + (2 * P0.z - 5 * P1.z + 4 * P2.z - P3.z) * pow(t, 2) +
